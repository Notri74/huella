...
const currentCarritoId = computed(() => {
  let id = storeCarrito.createId(menuItem.year, menuItem.iso, menuItem.language);
  ...
  <div class="container p-3">
    <div class="d-flex">
      <div class="form-group" v-for="(reactiveVar, reactiveKey) in reactiveData">
        <label v-if="menuItem.language.toLocaleUpperCase() == 'UK'" class="form-label mt-4">{{ titles.en[reactiveKey]
        }}</label>
        <label v-else class="form-label mt-4">
          {{ titles.es[reactiveKey] }}
        </label>
        <select class="form-select" v-model="reactiveData[reactiveKey]" :key="reactiveKey">
    ...
    <div>
      <div class="form-group">
        <div class="input-group mb-3">
          <input class="form-control" type="number" v-model="value" placeholder="Introduce un valor" />
        ...
        <input class="form-control" type="text" v-model="description" placeholder="Introduce una descripción" required />
