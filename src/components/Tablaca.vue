...
const items_scoped_values = computed(() => {
  ...
  Object.keys(items_scoped.value).map((scope) => {
    ...
    items_scoped.value[scope].map((item) => {
      values[scope].push(
        calculo(item.valorGhg, item.value)
      );
    });
  });
  return values;
...
const items_values3 = computed(() => {
  ...
  Object.keys(items_values2.value).map((scope: any) => {
    ...
    let calculoN2O =
      calculo(
        get_ghg_value(items_values2.value[scope].id, 'kg N2O', items_values2.value[scope].year),
    ...
    let calculoCO2 =
      calculo(
        get_ghg_value(items_values2.value[scope].id, 'kg CO2' , items_values2.value[scope].year),
        items_values2.value[scope].value)
    ...
    let calculoCH4 =
      calculo(
        get_ghg_value(items_values2.value[scope].id, 'kg CH4', items_values2.value[scope].year),
        items_values2.value[scope].value)
  ...
  <table class="table table-hover" v-if="props.carrito">
    <tbody scope="row" v-for="(i_scope, ind) in items_scoped" :key="ind">
      ...
      <tr v-for="(i_carr, index) in i_scope" :key="index">
        ...
        <td>
          <ul>
            <li v-for="val in get_ghgs_by_id(i_carr.id, i_carr.year)" :key="val">
