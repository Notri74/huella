    ...
    for (let scope of Object.keys(tablaScope)) {
      ...
      for (let i = 0; i < tablaBusqueda.length; i++) {
        let resultado: number = calculo(
          ...
          tablaBusqueda[i].valorGhg, tablaBusqueda[i].value
        );
...
<template>
  ...
  <ul class="nav nav-tabs">
    ...
    <li class="nav-item btn btn-outline">
      <a :class="['nav-link']" href="#" @click="storeCarrito.deleteCarritos()" >Borrar datos</a>
    </li>
    
   
    <li class="nav-item btn btn-outline" v-for="(item, index) in carrito" :key="index">
      <a :class="['nav-link', currentCarrito == (index as string) ? 'active' : '']" href="#"
        @click="setCurrentCarrito(index), changePage('Tablaca',index)">{{ index }} </a>
       <a v-if="currentCarrito ==  index "  class="btn btn-info"  @click="loadCarritoPage(index)">Añadir datos</a>
  ...
  <Tablaca :carrito="currentCarrito" v-if="currentCarrito && page!='Calculadora'" ></Tablaca>
